  ...
  return (
    ...
    >
      ...
      <div className="w-full relative">
        ...
        <div
          className="mx-auto w-full max-w-[1440px] flex justify-center items-center gap-3 sm:gap-6 select-none px-1.5 sm:px-3"
          {...handlers}
        >
          {windowIdx.map((idx, i) => {
            ...
            return (
              <motion.article
                ...
                w-[min(88vw,360px)] sm:w-[420px]
                h-[520px] sm:h-[580px]
                ${i === 1 ? "scale-100 sm:scale-105 z-20" : "scale-95 opacity-85 z-10"}`}
                style={{
                  pointerEvents: i === 1 ? "auto" : "none",
                }}
                whileHover={i === 1 ? { scale: 1.07 } : {}}
              >
                {/* Top visual (mobile lebih pendek) */}
                <div className="h-[220px] sm:h-[260px]">
                  <ProjectVisual project={p} />
                </div>

                {/* Bottom content (mobile lebih lega) */}
                <div className="flex-1 flex flex-col p-5 sm:p-6 pb-8 sm:pb-12 text-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300 leading-snug mx-auto max-w-[44ch] max-h-[4.2rem] sm:max-h-[3.6rem] overflow-hidden">
                      {p.desc}
                    </p>

                    {(p.highlights?.length || 0) > 0 ? (
                      <div className="mt-4 sm:mt-5 mb-5 sm:mb-7 flex flex-wrap gap-2 justify-center max-h-[4.6rem] sm:max-h-[4.2rem] overflow-hidden">
                        {chips.map((t) => (
                          <span
                            key={t}
                            className="text-[11px] sm:text-xs px-3 py-1 rounded-full bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-gray-200 border border-black/5 dark:border-white/10"
                          >
                            {t}
                          </span>
                        ))}
                        {remaining > 0 && (
                          <span className="text-[11px] sm:text-xs px-3 py-1 rounded-full bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-gray-200 border border-black/5 dark:border-white/10">
                            +{remaining} more
                          </span>
                        )}
                      </div>
